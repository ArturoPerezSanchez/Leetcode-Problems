...
class Solution:
    '''
	Time complexity: O(2^n), where n is length of the group list, since the recursive function performs contant operations and its called 2 times per level. Aditionally,
					 the creation of the list of tuples l takes O(nlogn) time to create since the list lenght is n and the sorting algorithm takes (nlogn) to sort the whole list. 

	Space complexity: O(n), where n is length of the group list, since the recursion tree can have at most n levels. Aditionally we also create a list of tuples of n elements.
    '''
